...
export const updateAdmin_controller = async (req, res) => {
  const { id } = req.params;
  const { name, email, password } = req.body; // Incluye el campo de contraseña
  ...
  try {
    ...
    // Si hay una contraseña, encriptarla antes de actualizar
    let hashedPassword = null;
    if (password) {
      const saltRounds = 10; // Ajusta según el nivel de seguridad deseado
    ...
    const updatedAdmin = await adminModel.updateAdminById(id, { 
      name, 
      email, 
      ...(hashedPassword && { password: hashedPassword }) // Agrega la contraseña solo si existe
    });
